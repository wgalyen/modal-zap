...
const Common = {
  ...
  inject(func, ...args) {
    return new Promise((resolve) => {
      ...
      script.dataset.modalzap = 'true'

      script.onload = () => {
        const receiveMessage = ({ data: { modalzapResponse } }) => {
          if (!modalzapResponse || modalzapResponse.uid !== uid) {
            return
          }
          ...
          resolve(modalzapResponse.message)
        }

        window.addEventListener('message', receiveMessage)

        window.postMessage({ modalzapRequest: { func, args, uid } })
      }
